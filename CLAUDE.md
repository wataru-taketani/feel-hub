...
### プリデプロイQA（push前に必須実行）
過去の障害は全てビルドで検出不可能なランタイムのデータ不整合。`npm run build` 成功後、`git push` 前に以下を実行する。

#### 1. データ契約検証（DB操作を含む変更時は必須）
Supabase execute_sql で実データとコードの整合性を確認する。
- **カラム存在チェック**: コード内の `.eq()` / `.select()` で使うカラム名が `information_schema.columns` に実在するか
- **値フォーマット突合**: 異なるテーブルの値を比較・結合する場合、`SELECT DISTINCT <column> FROM <table> LIMIT 20` を**両方のテーブルで実行**し、フォーマットの一致を確認する（例: `lessons.studio = "川崎"` vs `user_reservations.studio = "川崎（KWS）"` の差異検出）
- **ベースライン記録**: 変更に関連する `SELECT count(*)` を記録し、デプロイ後検証で使う

#### 2. 実データトレース（ロジック変更時は必須）
変更したコードに本番データのサンプル値を当てはめ、出力を追跡する。
- Supabase execute_sql でサンプルデータを1-3件取得
- コードの各ステップで変数がどう変化するかを追跡
- 最終出力が期待値と一致することを確認
- 例: Map のキー構成を変更 → 実データでキーを組み立て → 参照先テーブルの値とマッチするか検証

#### 3. 影響範囲分析（全変更で必須）
- 変更した関数/変数名で grep → 呼び出し元を列挙し、意図しない影響がないか確認
- 変更したテーブルを `.from('<table>')` で grep → 同テーブルの他クエリとパターンが一致するか確認
- 既存APIに確立されたパターン（getFcSession/reauthSession 等）がある場合、新コードも踏襲しているか確認

#### 4. チームレビュー（M規模以上は必須）
サブエージェントで以下の**全観点**を検証する。コード正確性だけでは不十分。
- **要件一致**: ユーザーの指示文を引用し、実装と1対1で対応を検証
- **データ契約**: コードが仮定するDB構造/値フォーマット vs 実DB。上記1のSQL結果を根拠に判定
- **回帰影響**: 既存機能が壊れないか。上記3の影響範囲分析を根拠に判定
- **devil's advocate**: 「この変更で本番が壊れるシナリオ」を列挙し、全て反論できることを確認
