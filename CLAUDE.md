...
**規模判定ルール**: DB操作を新規追加する変更は、ファイル数に関わらず最低M。**異なるデータソース間のフィールド値を比較・結合する変更**も最低M（フォーマット差異によるサイレント障害リスク）。迷ったら上の規模を選ぶ。M/L規模をSとして扱うと、サイレント障害（PostgRESTの空結果等）を見逃す。

### 本番環境が正
- **本番環境（Amplify / Lambda）を唯一の検証環境とする**。ローカル動作確認は不要
...
5. **異なるテーブル/データソースのフィールド値を比較・結合する場合**: 両方のテーブルから `SELECT DISTINCT <column> FROM <table> LIMIT 20` を実行し、実際の値フォーマットが一致することを確認する。「同じカラム名だから同じフォーマットだろう」は禁止。フォーマットが異なる場合は正規化関数を適用するか、ID（UUID）による結合に切り替える

### コミット＆デプロイフロー
コード変更後は以下を必ず順に実行すること：
1. `npm run build` — ビルドエラーがないことを確認
2. **プリデプロイQA（次セクション参照）** — push前に必ず実行。ビルド成功はQAではない
3. Supabaseマイグレーションがある場合: **コードデプロイの前に** `npx supabase db push`（新カラム追加→コードデプロイの順が安全。逆だと新コードが古いスキーマで動く）
4. `git add <変更ファイル>` + `git commit` — 変更をコミット
5. `git push origin main` — Amplify 自動デプロイをトリガー
6. **デプロイ後検証（後述）** — 検証完了してからユーザーに報告する
...
### プリデプロイQA（push前に必須実行）
過去の障害は全てビルドで検出不可能なランタイムのデータ不整合。`npm run build` 成功後、`git push` 前に以下を実行し、**結果をユーザーに提示**してから push する。

#### 1. データ契約検証（DB操作・データ比較を含む変更時は必須）
Supabase execute_sql で実データとコードの整合性を確認する。**SQLの実行結果をユーザーに提示**すること。
- **カラム存在チェック**: コード内の `.eq('xxx')` / `.select('xxx')` の `xxx` が実テーブルに存在するか確認
  ```sql
  SELECT column_name FROM information_schema.columns WHERE table_name = '<table>';
  ```
  コード内の全カラム名を結果と突合し、1つでも不在なら push 禁止
- **値フォーマット突合**: 異なるテーブルの値を比較・結合する場合
  ```sql
  SELECT DISTINCT <column> FROM <table_a> LIMIT 20;
  SELECT DISTINCT <column> FROM <table_b> LIMIT 20;
  ```
  両方の結果を並べて提示し、フォーマットが一致しない場合は UUID 結合に切り替える
- **ベースライン記録**: 変更に関連する COUNT を記録（デプロイ後に同じクエリで回帰検証）
  ```sql
  SELECT count(*) FROM <table> WHERE <変更に関連する条件>;
  ```

#### 2. 実データトレース（ロジック変更時は必須）
変更したコードを**1行ずつ**本番データで追跡し、**トレース結果をユーザーに提示**する。

**手順**:
1. Supabase execute_sql でサンプルデータを取得（マッチすべきレコードとマッチすべきでないレコードの両方）
2. 変更したコードの各行について、変数の値を書き出す
3. 期待する結果（true/false, マッチ/非マッチ）と実際の結果が一致するか確認

**記述フォーマット**（この形式で提示すること）:
```
■ ケース1: マッチすべきレコード
  入力: r = { lessonId: "b41614a9", programName: "April FEEL", studio: "川崎（KWS）" }
  L155: r.lessonId = "b41614a9" → truthy → if ブロック実行
  L156: map.set("b41614a9", "13") → map = {"b41614a9": "13"}
  L158: else → スキップ
  判定: lesson.id = "b41614a9" → map.has("b41614a9") = true ✓ 期待通り

■ ケース2: マッチすべきでないレコード
  対象: lesson = { id: "xxx-other", programName: "April FEEL", studio: "新宿" }
  判定: map.has("xxx-other") = false, フォールバックキーは map に存在しない → false ✓ 期待通り
```

**「やった」と言うだけでは不可** — 上記フォーマットの出力がなければトレース未実施とみなす。

#### 3. 影響範囲分析（全変更で必須）
以下を**実行結果とともに**ユーザーに提示する。
- 変更した関数/変数名で grep → 呼び出し元のファイル:行番号を列挙
- 変更したテーブルを `.from('<table>')` で grep → 同テーブルの他クエリのパターンを確認
- 既存APIに確立されたパターン（getFcSession/reauthSession 等）がある場合、新コードも踏襲しているか
- **チームエージェントの提案と自分の実装に差分がある場合、その差分を明示し理由を説明する**

#### 4. チームレビュー（M規模以上は必須）
サブエージェントに**変更後のコード全文**（diffではなく）を渡し、以下の全観点で検証する。
- **要件一致**: ユーザーの指示文を引用し、実装と1対1で対応を検証
- **データ契約**: コードが仮定するDB構造/値フォーマット vs 上記1のSQL結果
- **回帰影響**: 上記3の影響範囲に基づき、既存機能が壊れないか検証
- **devil's advocate**: 「この変更で本番が壊れるシナリオ」を3つ以上列挙し、全て反論できることを確認
- レビュー結果は **pass/fail を明示**。1つでも fail なら push 禁止
...
- **Amplify無料枠（ビルド1000分/月）**: 無駄なデプロイは避ける
...
- **ホスティング**: AWS Amplify (フロント) + AWS Lambda (定期実行)
...
- Amplify env vars に改行が混入しないよう注意
- API Route内で `request.nextUrl.origin` は使わない（Amplify SSRでは `localhost:3000` を返す）。代わりに `x-forwarded-host` / `host` ヘッダーから取得する
