...
### コーディング前チェック（DB操作を含む変更時は必須）
DB操作（Supabaseクエリ）を新規追加・変更する場合、コーディング前に以下を実行すること：
1. **対象テーブルのカラム名を確認**: `SELECT column_name FROM information_schema.columns WHERE table_name = '<table>'` で実カラム名を取得
...
1. **API/DB変更時 — 本番データ検証（最優先）**: 変更したAPIが実際にデータを返すことを確認する
   - Supabase execute_sql で `SELECT count(*) FROM <table> WHERE <condition>` を実行し、空でないことを検証
   - 新規テーブル・カラム追加時は、データが正しく書き込まれていることを確認
   - **ビルド成功 ≠ 動作確認。DBスキーマ不整合はビルドで検出できない**
2. **MEMORY.md 更新**: 新しく判明した事実（API仕様、バグ原因、アーキテクチャ変更等）があれば更新
3. **CLAUDE.md TODO 更新**: 完了したTODOはチェック済み/削除、新規TODOは追記
4. **Lambda変更時**: `aws logs tail /aws/lambda/feel-hub-lambda-prod-checkCancellation --since 3m --format short` でログ確認
5. **Vercel変更時**: デプロイ正常完了を確認
