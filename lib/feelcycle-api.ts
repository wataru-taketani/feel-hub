...
/**
 * 全プログラム一覧を取得（POST /api/reserve/lesson_program）
 */
export async function getProgramList(session: FeelcycleSession): Promise<ProgramSeries[]> {
  const res = await fetch(`${BASE_URL}/api/reserve/lesson_program`, {
    method: 'POST',
    headers: buildHeaders(session),
  });
  ...
  if (!res.ok) {
    throw new Error(`プログラム一覧取得失敗 (${res.status})`);
  }

  const data = await res.json();
  const programList = (data.program_list || []) as {
    series_name: string;
    series_program_list: { id: number; name: string }[][];
  }[];

  return programList.map((series) => ({
    seriesName: String(series.series_name || ''),
    programs: (series.series_program_list || [])
      .flat()
